...
########################################################################################################################
# The bash script for installing the commit-msg hook for MacOS systems                                                 #
#                                                                                                                      #
# @author devonline                                                                                                    #
...
# SCRIPT=https://raw.githubusercontent.com/devonline-academy/tools/master/git/commit-msg-verifier/setup-mac.sh         #
...
# ----------------------------------------------------------------------------------------------------------------------
# Verify that the curl is available:
set +e
CURL_CMD=$(which curl)
set -e
if [[ ! -x "$CURL_CMD" ]]; then
  echo "------------------------------------------------------------------------" >&2
  echo "Download and add \"curl\" to the \"PATH\" variable, before using this script!" >&2
  echo "------------------------------------------------------------------------" >&2
  exit 3
fi
# ----------------------------------------------------------------------------------------------------------------------
...
curl -o ${HOOKS_DIR}/${HOOK_SCRIPT_NAME}.java ${CURL_QUITE} ${DOWNLOAD_HOOK_SCRIPT_URL}
cd ${HOOKS_DIR}
...
if [[ ! -f "$VERBS_FILE_PATH" ]]; then
  ...
  curl -o "${VERBS_FILE_PATH}" ${CURL_QUITE} ${DOWNLOAD_VERBS_FILE_URL}
